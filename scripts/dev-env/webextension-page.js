  ...
  runtime: {
    ...
    onMessage: {
      ...
      removeListener (listener) {
        ...
        window.msgPageListeners = _.filter(
          window.msgPageListeners,
          x => x !== listener
        )
      },
...
function genStorageApis () {
  ...
  return {
    ...
    onChanged: {
      ...
      removeListener (listener) {
        ...
        storageData.listeners = _.filter(
          storageData.listeners,
          x => x !== listener
        )
      },
      hasListener (listener) {
  ...
  function genStorageAreaApis (area) {
    return {
      ...
      getBytesInUse (keys) {
        if (_.isNull(keys)) {
          return Promise.resolve(
            new Blob([JSON.stringify(storageData[area])]).size
          )
        }
        if (_.isString(keys)) {
          keys = keys ? [keys] : []
        ...
        return Promise.resolve(
          new Blob([JSON.stringify(_.pick(storageData[area], keys))]).size
        )
      }
    }
  }
  ...
  function alertListeners (changes, area) {
    storageData.listeners.forEach(listener =>
